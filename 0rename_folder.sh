...
for i in `seq 0 66`
do
	if [ -d ./lab_5/${csdaccount[$i]} ]
	then
		echo  ./lab_5/${csdaccount[$i]} ./lab_5/${name[$i]} ./lab_5/${name[$i]}_${csdaccount[$i]}
 		mv ./lab_5/${csdaccount[$i]} ./lab_5/${name[$i]}_${csdaccount[$i]}
	else
		mkdir ./lab_5/${name[$i]}_${csdaccount[$i]}
		touch ./lab_5/${name[$i]}_${csdaccount[$i]}/NONE_SUBMISSION
	fi	
done